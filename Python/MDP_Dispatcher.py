...
if __name__ == '__main__':
	...
	Islands=['Zone_19','Zone_20','Zone_21','Zone_47','Zone_48']

	Avaiable_Zones=[l for l in Zone_list if l not in Islands]

	Driver_df['Zone']=Driver_df.apply(lambda x:random.choice(Avaiable_Zones) if x['Zone'] in Islands else x['Zone'], axis=1)
	...
	for step in range(Start_step,End_step,1):
	    ...
	    Next_Driver_df['Tuple']=Next_Driver_df.apply(lambda x:reposition(x['Point'],x['Zone'],x['Reposition_Point'],speed,Connect_matrix,Point_coordinate,G,Zone_Point,x['Step'],Policy),axis=1)
